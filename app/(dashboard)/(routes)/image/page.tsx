...
const ImagePage = () => {
    ...
    const onSubmit = async (values: z.infer<typeof formSchema>) => {
        try {
            // Reset all the photos every time you click submit
            setImages([]);

            // Create an API Call
            const response = await axios.post("/api/image", values);

            // Extract the urls for the images from the api's resposnse
            const urls = response.data.map((images: {url: string}) => images.url);

            // Assign the 'setIamges' variable to the received urls
            setImages(urls);
